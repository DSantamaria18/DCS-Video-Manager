...
| # | Title | Priority | Description |
| --- | --- | --- | --- |
| 8 | Descriptions adapted to video length | 🔴 High | Gemini must receive video duration before generating metadata and produce structurally different outputs per length: <10 min → "quick tactical breakdown" (brief intro + key takeaway + links); 10–30 min → "full training video" (intro + chapter breakdown + detailed links); >30 min → "complete mission debrief" (brief + mission summary + detailed breakdown + bonus material). |
...
| 36 | Automated playlist assignment | 🟡 Medium | After analysis, automatically match the detected aircraft and mission type against the channel's existing playlist names (already fetched via `playlists().list()`). Pre-select matching playlists in the upload UI (user can override before confirming). Example: F-18C + BVR → auto-selects "F-18C Advanced" and "BVR Master Class" if they exist on the channel. Minimal code on top of the existing playlist selector. |
...
| 42 | Description SEO optimizer | 🟡 Medium | After the initial metadata generation, run a second lightweight validation pass: check for missing high-value keywords (aircraft full name, mission type, "DCS World"), flag descriptions shorter than 300 chars, verify that chapter timestamps appear near the top, check that the most relevant playlist link is in the first 100 chars. Optionally send flagged issues back to Gemini for a targeted rewrite. Displayed as inline suggestions in the description editor. |
