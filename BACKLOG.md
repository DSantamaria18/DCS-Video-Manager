...
| TEC-05 | 129 issues de `ruff` sin arreglar, CI no bloquea por ellas | P2 | M | `ruff check .` (añadido en INF-01/INF-03) reporta 129 issues (45 auto-fixables con `--fix`, 19 más con `--unsafe-fixes`). El paso de lint en `.github/workflows/ci.yml` corre con `|| true` para no dejar el CI en rojo desde el día 1 (ver `DECISIONS.md`). Cuando lleguen a cero, quitar el `|| true` y ruff pasa a bloqueante. |
