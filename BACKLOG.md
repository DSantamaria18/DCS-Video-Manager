...
| ~~TEC-01a~~ | ~~Extraer `acmi/` de `dcs_meta.py`~~ | P1 | S | **Hecho.** `_parse_acmi_props`, `parse_acmi_events` movidos a `acmi.py`, reexportados desde `dcs_meta.py`. Ver `CHANGELOG.md`. |
| ~~TEC-01b~~ | ~~Extraer `thumbnail/` de `dcs_meta.py`~~ | P1 | S | **Hecho.** `_load_font`, `_fit_text`, `_score_frame`, `_grade_frame`, `_apply_thumbnail_overlay`, `_save_thumbnail`, `generate_thumbnail_on_demand` movidos a `thumbnail.py`. Ver `CHANGELOG.md`. |
