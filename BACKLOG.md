...
| INF-04 | Excluir `.claude/worktrees/` de la configuración del linter | P1 | S | Parte de pytest ya hecha: `pytest.ini` (`norecursedirs`) y `.gitignore`, verificado con `pytest --collect-only` (233 tests, cero recolección desde worktrees). Falta el `exclude` del linter — bloqueado por INF-03: sin herramienta de linting elegida (pregunta 3 de `SPEC.md` sin responder). |
