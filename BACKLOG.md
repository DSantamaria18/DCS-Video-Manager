...
| 48 | History tab rows not clickable | 🔴 High | — | Clicking a row in the History tab has no effect. Feature #24 was implemented but the click handler or the detail panel reveal logic is not working. Either the event listener is not attached, the panel element is missing, or a JS error prevents the handler from running. |
