...
- **Discord webhook notification** — after a successful upload, if `discord_webhook_url` is configured (stored in `config/secrets.json`, see Infrastructure), an embed (title, URL, description excerpt) is POSTed to the Discord webhook using only `urllib.request` (no extra deps). Non-fatal: logs and continues if the webhook fails.
...
- **`config/config.json`** — channel name, default links (playlists, social, sponsorship, squadron), number of frames to extract, Gemini model, `recordings_folder` (for batch watcher). Discord secrets (`discord_webhook_url`, `discord_bot_token`, `discord_channel_id`) live in a separate gitignored `config/secrets.json` (SEC-01), merged in transparently by `dcs_meta.load_config()`.
